...
$constWlbYamlUrl = 'https://raw.githubusercontent.com/bgeveritt/logzio-winlogbeat-deploy/master/winlogbeat.yml'
...
((Get-Content -Path $wlbYaml -Raw) -Replace '<<LOGZ.IO TOKEN>>', $token -Replace '<<LOGZ.IO LISTENER>>', $listener -Replace '<<LOGZ.IO CERTIFICATE>>', ('{0}{1}' -f $wlbHome, (Split-Path $constLogzioCertificate -Leaf))) | Set-Content -Path $wlbYaml
